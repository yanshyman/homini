...
module homini.controllers {
    ...
    export class LoginController {
        ...
        public getStatus() {
            this.$http.get("https://api.thingspeak.com/channels/346301/feeds.json?api_key=LEAJEWO1097UDGKH&results=50").then((response: any) => {
                if (response) {
                    this.$scope.currentTemperature = response.data.feeds[0].field1;
                    this.$scope.isHeating = response.data.feeds[0].field2;
                    this.$scope.onOff = "LOGIN";
                    this.$scope.lastUpdate = moment(response.data.feeds[0].created_at).format("DD-MM-YYYY HH:mm");
                }
            },
                (error) => { });
        ...
        public getWheather() {
            this.$http.get("http://api.openweathermap.org/data/2.5/weather?q=Gdynia&appid=3c486328acf637551b0f68c8ca5f90e4").then((response: any) => {
                if (response) {

                    this.$scope.temperatureOutside = response.data.main.temp - 272.15;
